...
public class OpenBitSet implements IBitSet {
	 ...
	 * caused by attempting to allocate large, contiguous arrays
	 * (CASSANDRA-2466). All sub-arrays but the last are uniformly PAGE_SIZE
	 * words; to avoid waste in small bloom filters (of which Cassandra has
	 * many: one per row) the last sub-array is sized to exactly the remaining
	 * number of words required to achieve the desired set size
	 * (CASSANDRA-3618).
	 */
	...
	public void intersect(OpenBitSet other) {
		...
		int pos = newLen;
		while (--pos >= 0) {
			thisArr[pos / thisPageSize][pos % thisPageSize] &= otherArr[pos
					/ otherPageSize][pos % otherPageSize];
		}
	...
	public boolean equals(Object o) {
		...
		for (int i = b.wlen - 1; i >= 0; i--) {
			if (a.bits[i / aPageSize][i % aPageSize] != b.bits[i / bPageSize][i
					% bPageSize])
				return false;
		}
